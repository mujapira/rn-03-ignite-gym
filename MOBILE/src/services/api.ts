...
type APIInstanceProps = AxiosInstance & {
  TokenGuardian: (signOut: SignOut) => () => void
}
...
api.TokenGuardian = signOut => {
  const tokenGuardian = api.interceptors.response.use(
    response => response,
    async requestError => {
  ...
  return () => {
    api.interceptors.response.eject(tokenGuardian)
  }
}
